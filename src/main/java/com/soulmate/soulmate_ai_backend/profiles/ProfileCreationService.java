...
import static com.soulmate.soulmate_ai_backend.Utils.generateMyersBriggsTypes;
import static com.soulmate.soulmate_ai_backend.Utils.selfieTypes;
...
public class ProfileCreationService {
    ...
    public ProfileCreationService(VertexAiGeminiChatModel chatClient, ProfileRepository profileRepository) {
    ...
    public void createProfiles(int numberOfProfiles) {
        ...
        while (this.generatedProfiles.size() < numberOfProfiles) {
            ...
            ChatResponse response = chatClient.call(new Prompt(prompt,
                    VertexAiGeminiChatOptions.builder().withFunction("saveProfile").build()));
    ...
    private Profile generateProfileImage(Profile profile) {
        ...
        String prompt = """
                Selfie of a %d year old %s %s %s, %s, photorealistic skin texture and details, \
                individual hairs and pores visible, highly detailed, photorealistic, hyperrealistic, \
                subsurface scattering, 4k DSLR, ultrarealistic, best quality, masterpiece. Bio- %s
                """.formatted(
                profile.age(),
                profile.myersBriggsPersonalityType(),
                profile.ethnicity(),
        ...
        String negativePrompt = """
                multiple faces, lowres, text, error, cropped, worst quality, low quality, jpeg artifacts, \
                ugly, duplicate, morbid, mutilated, out of frame, extra fingers, mutated hands, poorly \
                ...
                neck, username, watermark, signature
                """;
        ...
        // Printing the output
        System.out.println("""
                Creating image for %s %s (%s)
                """.formatted(profile.firstName(), profile.lastName(), profile.ethnicity()));
